...
if not df.empty:
    ...
    # 1. 사용자에게 프롬프트로 파일명 입력받기
    filename = input(
        "💾 저장할 CSV 파일명을 입력하세요 (예: lotte_vouchers): "
    ).strip()

    # 2. 사용자가 아무것도 입력 안 했을 때를 대비한 기본값 설정
    if not filename:
        filename = "lotte_hotel_vouchers_default"

    # 3. 사용자가 뒤에 '.csv'를 붙여서 입력했는지 체크하고 보정
    if not filename.endswith(".csv"):
        filename += ".csv"

    # 4. 저장할 폴더('data')가 없으면 자동으로 만들어주는 안전장치
    folder_path = "data"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
        print(f"📁 '{folder_path}' 폴더가 존재하지 않아 새로 생성했습니다.")

    # 5. 폴더 경로와 파일명을 결합 (슬래시 '/' 사용으로 에러 방지)
    final_path = f"{folder_path}/{filename}"

    # 6. 구글 시트/엑셀 한글 깨짐 방지 옵션을 넣어 CSV 저장
    df.to_csv(final_path, index=False, encoding="utf-8-sig")

    print("-" * 50)
    print(f"🎉 성공적으로 저장되었습니다!")
    print(f"📍 저장 위치: {os.path.abspath(final_path)}")
    print("-" * 50)
else:
    print("수집된 데이터가 없습니다.")