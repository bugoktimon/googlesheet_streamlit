...
TARGET_SPREADSHEET_NAME = "스크래핑에서데이터저장시각화"
...
def upload_csv_to_sheets():
    ...
    try:
        # 3. 공통 모듈을 이용해 구글 시트 접속 (인증 로직 분리 적용)
        spreadsheet = get_spreadsheet(TARGET_SPREADSHEET_NAME, CREDENTIALS_FILE)
        ...
        print(" 2. 실제 구글 시트 우측 상단 '공유' 버튼을 눌러, credentials.json 내부의 서비스 계정 이메일을 '편집자'로 추가했는지 꼭 확인하세요.")
